 ...
 */
    ...
    TFile* file1 = new TFile("~/2014Research/ROOT_file/V0reco_pPb_rpyDependent_3Dhisto/HMRereco_TH3D_Sep23_rpyDependent_v1_2014.root");
   ...
   double la_norm[5];
    ...
    double rpybins[6] = {63,163,260,350,443,543};
    ...
    for (int mult = 0; mult < 1; mult++){
        ...
        for (int rpy = 0; rpy < 5; rpy++){
            ...
            for (int pt = 3; pt < 15; pt++){
                ...
                ks_HM[mult][rpy][pt] = ksHist[mult]->ProjectionZ( ksHistName.str().c_str(),rpybins[rpy]+1,rpybins[rpy+1],pTbinsBound_1[pt]+1,pTbinsBound_1[pt+1] );
                la_HM[mult][rpy][pt] = laHist[mult]->ProjectionZ( laHistName.str().c_str(),rpybins[rpy]+1,rpybins[rpy+1],pTbinsBound_1[pt]+1,pTbinsBound_1[pt+1] );
                xi_HM[mult][rpy][pt] = xiHist[mult]->ProjectionZ( xiHistName.str().c_str(),rpybins[rpy]+1,rpybins[rpy+1],pTbinsBound_1[pt]+1,pTbinsBound_1[pt+1] );
            }
        }
    }
 ...
 */
    ...
    TFile* t1 = new TFile("~/2014Research/Code/Jet'study/gitV0sRatio/hijingEfficiencyRapidityTable/effKongNew2DTable_10M_Sep22_rapidity_v1_12pTbins.root");
 ...
 */
    ...
    for (mult = 0; mult < 1; mult++){
        ...
        ksSpectra[mult][rpy] = new TH1D(ksHistName.str().c_str(),ksHistName.str().c_str(),15,ptbins_1);
        laSpectra[mult][rpy] = new TH1D(laHistName.str().c_str(),laHistName.str().c_str(),15,ptbins_1);
        ratioHist[mult][rpy] = new TH1D(ratioHistName.str().c_str(),ratioHistName.str().c_str(),15,ptbins_1);
    
            for (pt = 3; pt < 15; pt++){

                double ks_temp = (ks_HM_yield[mult][rpy][pt]/binwidth_1[pt])/(2*3.1415926*ptbins_1[pt+1]*rpybinwidth[rpy]*ks_norm[mult]);
                double la_temp = (la_HM_yield[mult][rpy][pt]/binwidth_1[pt])/(2*3.1415926*ptbins_1[pt+1]*rpybinwidth[rpy]*la_norm[mult]);

                ksSpectra[mult][rpy]->SetBinContent(pt+1, ks_temp );
                ksSpectra[mult][rpy]->SetBinError(pt+1, temp_ks_err[mult][rpy][pt]/( 2*3.1415926*binwidth_1[pt]*ptbins_1[pt+1]*rpybinwidth[rpy]*ks_norm[mult] ));

                laSpectra[mult][rpy]->SetBinContent(pt+1, la_temp );
                laSpectra[mult][rpy]->SetBinError(pt+1, temp_la_err[mult][rpy][pt]/( 2*3.1415926*binwidth_1[pt]*ptbins_1[pt+1]*rpybinwidth[rpy]*la_norm[mult] ));
    ...
    TFile f1("HMbins_12pTBins5rpyBins_wErr_withoutEfficiencyCheck.root","new");
